 ...
 */
public class Wahlperiode_MongoDB_Impl extends Wahlkreis_Impl implements Wahlkreis {
    private database.MongoDBHandler MongoDBHandler;
    private int MongoID_match;
    private String sCollection;
    private Document docu;

    public Wahlperiode_MongoDB_Impl(database.MongoDBHandler MongoDBHandler, int MongoID_match){
        this.MongoDBHandler = MongoDBHandler;
        this.MongoID_match = MongoID_match;
        this.sCollection = "Wahlperiode";
        this.docu = MongoDBHandler.getDocumentByMongoID(this.sCollection,this.MongoID_match);
    }


    public int getWP_Nummer() {
        if (this.docu.getInteger("WP_Nummer")==null){
            return 0;
        } else {
            return this.docu.getInteger("WP_Nummer");
        }
    ...
    public List<Integer> getAbgeordneten_IDs() {
        ...
        FindIterable<Document> Cursor = pCollection.find(searchQuery);
        for (Document o : Cursor) {
            Inhalt = (List<Integer>) o.get("Abgeordneten_IDs");
        }
